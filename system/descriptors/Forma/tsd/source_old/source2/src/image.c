...
Image  *CopySubImage(Image *img, 
  ...
  if(x1>x2 || y1>y2)
    Error("Invalid coordinates","CopySubImage");

  sub = CreateImage(x2-x1+1, y2-y1+1);
  i=0;
  for(y=y1; y<=y2; y++){
    for(x=x1; x<=x2; x++){
...
void DrawImageLineDDA(Image *img, int x1, int y1, int xn, int yn, int val){  
  ...
  vy = yn - y1;
   ...
   for(i = 0; i < amostras; i++){ 
     if( ValidPixel(img, ROUND(xk), ROUND(yk)) ){
       p = ROUND(xk)+img->tbrow[ROUND(yk)];
       img->val[p] = val;
     }
     xk += Dx;
     yk += Dy;
...
Image *Rotate(Image *img, float theta) 
{
  ...
  for (v.y=0; v.y < rot->nrows; v.y++)
	...
	rot->val[v.x+rot->tbrow[v.y]] = ROUND(If);
      }
    }
...
Image *Zoom(Image *img, float Sx, float Sy) {
  ...
  scl = CreateImage((int)(img->ncols*Sx),(int)(img->nrows*Sy)); 
  ...
  for (v.y=0; v.y < scl->nrows; v.y++)
	...
	+ img->nrows/2.;
      ...
      u.y = (int)(y);
      if (ValidPixel(img,u.x,u.y)){
	scl->val[v.x+scl->tbrow[v.y]] = img->val[u.x+img->tbrow[u.y]];
      }
    }
