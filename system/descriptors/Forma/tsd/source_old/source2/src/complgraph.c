...
void ComplGraphSortLabel(ComplGraph **cg, char order)
{
	  ...
	  curve = CreateCurve(cgAux->n+1);

	  for (i = 0; i < cgAux->n; i++)
	  {
		  curve->X[i] = i;
		  curve->Y[i] = cgAux->node[i].label;
	  }
	  ...
	  for (i = 0; i< cgSort->n; i++)
		  GraphNodeCopy(&cgSort->node[i], &cgAux->node[(int)curve->X[i]]);
