...
Scene *CopyScene(Scene *scn){
  ...
  memcpy(aux->data,scn->data,sizeof(int) * n);
  return aux;
}
...
Scene *ReadScene(char *filename)
{
  ...
  if((strcmp(type,"SCN")==0)){
	...
	scn->data[i] = (int) data16[i];
      ...
      fread(data8,sizeof(uchar),2*n,fp);
      j=0;
      for (i=0; i < 2*n; i+=2) {
	...
	j++;
      ...
      */
      ...
      n = xsize*ysize*zsize;
      fread(scn->data,sizeof(int),n,fp);      
    }
    fclose(fp);
  } else {
...
int GetVoxelValue_trilinear(Scene *scn, double x,double y, double z)
{
...
int GetVoxelValue_nn(Scene *scn, double x, double y, double z)
{
  Voxel v;
  if (x>scn->xsize-1||y>scn->ysize-1||z>scn->zsize-1||x<0||y<0||z<0) {
    printf("GetVoxelValue_nn out-of-bounds\n"); exit(1);
  }
  v.x=(int)(x+0.5);
  v.y=(int)(y+0.5);
  v.z=(int)(z+0.5);
