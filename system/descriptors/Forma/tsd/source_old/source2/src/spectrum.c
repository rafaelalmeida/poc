...
Spectrum *FFT3D(Scene *scn)
{
  ...
  Spectrum *spec;
  int ncols, nrows, nslices, i, j,z;
  double *real, *imag;

  /* Enlarge ncols and nrows to the nearest power of 2 */
  ncols = (int)ceil(log(scn->xsize)/log(2));
  nrows = (int)ceil(log(scn->ysize)/log(2));
  nslices = (int)ceil(log(scn->zsize)/log(2));
  ncols = 1 << ncols;
  nrows = 1 << nrows;
	nslices =1 << nslices;

  spec = CreateSpectrum3D(ncols, nrows, nslices);
  ...
  for( z=0;z<nslices;z++){
  		...
  		for (i=0; i<spec->nrows; i++)
  		{
    		...
    		{
      		if (i<scn->ysize && j<scn->xsize)
        		real[j] = (double)scn->data[scn->tbz[z]+scn->tby[i]+j];
      		else
        		real[j] = 0.0;
      		imag[j] = 0.0;
    		}
    		...
    		for (j=0; j<spec->ncols; j++)
    		{
      		spec->real[spec->tbslice[z]+spec->tbrow[i]+j] = real[j];
      		spec->imag[spec->tbslice[z]+spec->tbrow[i]+j] = imag[j];
    		}
  		}
  		free(real);
  		free(imag);
  		...
  		for (i=0; i<spec->ncols; i++)
  		{
    		...
    		{
      		real[j] = spec->real[spec->tbslice[z]+ spec->tbrow[j]+i];
      		imag[j] = spec->imag[spec->tbslice[z]+ spec->tbrow[j]+i];
    		}
    		FFT(1, (long)spec->nrows, real, imag);
    		for (j=0; j<spec->nrows; j++)
    		{
      		spec->real[spec->tbslice[z]+spec->tbrow[j]+i] = real[j];
      		spec->imag[spec->tbslice[z]+spec->tbrow[j]+i] = imag[j];
    		}
  		}
  		free(real);
  		free(imag);
	...
	//Transform the slice orientation (Z)
	real = (double*)malloc(sizeof(double)*spec->nslices);
  imag = (double*)malloc(sizeof(double)*spec->nslices);
  if (real == NULL || imag == NULL)
  	Error(MSG1, "FFT2D");
  for (i=0; i<spec->nrows; i++)
  {
  	for (j=0; j<spec->ncols; j++)
    {
			for(z=0; z<spec->nslices;z++)
			{
				real[z]= spec->real[spec->tbslice[z] + spec->tbrow[i] + j];
				imag[z]= spec->imag[spec->tbslice[z] + spec->tbrow[i] + j];
    	}
    	FFT(1, (long)spec->nslices, real, imag);
    	for (z=0; z<spec->nslices; z++)
    	{
      		spec->real[spec->tbslice[z]+spec->tbrow[i]+j] = real[z];
      		spec->imag[spec->tbslice[z]+spec->tbrow[i]+j] = imag[z];
    	}
  	}
  }
  free(real);
...
Scene *InvFFT3D(Spectrum *spec)
{
  ...
  printf("entrei no inversa 3D!!!!!!!\n");
  scn = CreateScene(spec->ncols, spec->nrows, spec->nslices);
  specaux = CreateSpectrum3D(spec->ncols, spec->nrows, spec->nslices);
  printf("numero de fatias: %d\n",spec->nslices);
	for(z=0;z<spec->nslices;z++){
	  ...
	  for (i=0; i<spec->nrows; i++)
	  {
	    for (j=0; j<spec->ncols; j++)
	    {
	      real[j] = spec->real[spec->tbslice[z]+spec->tbrow[i]+j];
	      imag[j] = spec->imag[spec->tbslice[z]+spec->tbrow[i]+j];
	    }
	    FFT(-1, (long)spec->ncols, real, imag);
	    for (j=0; j<spec->ncols; j++)
	    {
	      specaux->real[spec->tbslice[z]+specaux->tbrow[i]+j] = real[j];
	      specaux->imag[spec->tbslice[z]+specaux->tbrow[i]+j] = imag[j];
	    }
	  }
	  free(real);
	  free(imag);
	  ...
	  for (i=0; i<specaux->ncols; i++)
	  {
	    for (j=0; j<specaux->nrows; j++)
	    {
	      real[j] = specaux->real[spec->tbslice[z]+specaux->tbrow[j]+i];
	      imag[j] = specaux->imag[spec->tbslice[z]+specaux->tbrow[j]+i];
	    }
	    FFT(-1, (long)specaux->nrows, real, imag);
	    for (j=0; j<specaux->nrows; j++)
	    {
	      specaux->real[spec->tbslice[z]+specaux->tbrow[j]+i] = real[j];
	      specaux->imag[spec->tbslice[z]+specaux->tbrow[j]+i] = imag[j];
	    }
	  }
	  free(real);
	  free(imag);
  ...
  for (i=0; i<spec->nrows; i++)
  {
  	for (j=0; j<spec->ncols; j++)
    {
			for(z=0; z<spec->nslices;z++)
			{
				real[z]= specaux->real[spec->tbslice[z] + spec->tbrow[i] + j];
				imag[z]= specaux->imag[spec->tbslice[z] + spec->tbrow[i] + j];
    	}
    	FFT(-1, (long)spec->nslices, real, imag);
    	for (z=0; z<spec->nslices; z++)
    	{
      		scn->data[scn->tbz[z]+scn->tby[i]+j] = (int)real[z];
    	}
  	}
  }
  free(real);
...
Spectrum *CreateSpectrum3D(int ncols, int nrows, int nslices)
{
  ...
  spec->real  = AllocDoubleArray(nrows*ncols*nslices);
  spec->imag  = AllocDoubleArray(nrows*ncols*nslices);
  spec->tbrow = AllocIntArray(nrows);
	spec->tbslice= AllocIntArray(nslices);

  for (i=0; i<nrows; i++)
    spec->tbrow[i]=i*ncols;
  for (i=0;i<nslices;i++)
  	spec->tbslice[i]=i*ncols*nrows;
...
void DestroySpectrum3D(Spectrum **spec)
{
  ...
  if (aux != NULL) {
    ...
    if (aux->imag != NULL)  free(aux->imag);
    if (aux->tbrow != NULL) free(aux->tbrow);
    if (aux->tbslice !=NULL) free(aux->tbslice);
    free(aux);
    *spec = NULL;
  }
}
...
Scene *ViewMagnitude3D(Spectrum *spec)
{
	...
	int i,z,n;
	scn=CreateScene(spec->ncols,spec->nrows,spec->nslices);
	spec2D=CreateSpectrum(spec->ncols,spec->nrows);
	n=spec->ncols*spec->nrows;
	for(z=0;z<spec->nslices;z++)
	{
		//copy Spectrum Slice
		for(i=0;i<n;i++){
			spec2D->real[i]=spec->real[spec->tbslice[z]+i];
			spec2D->imag[i]=spec->imag[spec->tbslice[z]+i];
		}
		img=ViewMagnitude(spec2D);
		PutSlice(img,scn,z);
	}
	DestroySpectrum(&spec2D);
	DestroyImage(&img);
	return(scn);
...
Spectrum *SpectrumMagnitude3D(Spectrum *spec)
{
  ...
  r = spec->nrows;
  s = spec->nslices;
  specmag = CreateSpectrum3D(c, r, s);
  ...
  /* Calculate the magnitude */
	...
	for(z=0; z<s ; z++){
  	for (i=0; i<r; i++) {
    	for (j=0; j<c; j++) {
    	  k = specmag->tbslice[z]+specmag->tbrow[i]+j;
    	  specmag->real[k] = sqrt(spec->real[k]*spec->real[k] + spec->imag[k]*spec->imag[k]);
    	}
  	}
  }
...
Spectrum *SpectrumPhase3D(Spectrum *spec)
{
  ...
  r = spec->nrows;
  s = spec->nslices;
  specphase = CreateSpectrum3D(c, r, s);

  /* Calculate the phase */
  for(z=0; z<s ; z++){
  	for (i=0; i<r; i++) {
  	  for (j=0; j<c; j++) {
  	    k = specphase->tbslice[z]+specphase->tbrow[i]+j;
  	    specphase->real[k] = atan(spec->imag[k]/spec->real[k]);
  	  }
  	}
	}
...
Spectrum *MultSpectrum3D(Spectrum *spec1, Spectrum *spec2)
{
  ...
  spec3=CreateSpectrum3D(spec1->ncols,spec1->nrows,spec1->nslices);
  n = spec1->ncols*spec1->nrows*spec1->nslices;
  for (p=0; p < n; p++) {
    spec3->real[p] = (spec1->real[p]*spec2->real[p]) - (spec1->imag[p]*spec2->imag[p]);
    spec3->imag[p] = (spec1->real[p]*spec2->imag[p]) + (spec1->imag[p]*spec2->real[p]);
  }
