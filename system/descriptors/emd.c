...
typedef FeatureVector1D* feature_t;
/*****************************************************************************/
...
{    
  ...
  int i, j, minI=NIL, minJ=NIL;

  /* FIND THE MINIMAL Cij-Ui-Vj OVER ALL i,j */
  deltaMin = MY_INFINITY;
  for(i=0; i < _n1; i++)
    for(j=0; j < _n2; j++)
      if (! _IsX[i][j])
	{
...
void russel(double *S, double *D)
{
  int i, j, found, minI=NIL, minJ=NIL;
